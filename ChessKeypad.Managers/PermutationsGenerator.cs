...
namespace ChessKeypad.Managers
{
    public class PermutationsGenerator
    {
        ...
        Dictionary<Coordinate, List<(Coordinate Coordinate, Cell Cell)>> CalculateValidMoves(Layout layout, Piece piece)
        {
            ...
            foreach (var coordinate in layout.Cells.Keys)
            {
                foreach (var move in piece.ValidMoves)
                {
                    ...
                    {
                        validMoves[coordinate] = new List<(Coordinate Coordinate, Cell Cell)>();
                    }
                    validMoves[coordinate].Add((endCoordinate, layout.Cells[endCoordinate]));
        ...
        int CalculatePermutations(int numberOfDigits, int position, Coordinate start, Cell cell, Dictionary<Coordinate, List<(Coordinate Coordinate, Cell Cell)>> validMoves)
        {
